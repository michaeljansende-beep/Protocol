...
(() => {
  ...
  async function copyResult(e) {
    try {
      // Falls als Event-Handler aufgerufen
      if (e && typeof e.preventDefault === "function") e.preventDefault();
      if (e && typeof e.stopPropagation === "function") e.stopPropagation();

      // iOS Notes/Mail verhalten sich beim programmatic Clipboard oft anders.
      // Deshalb: exakt wie "manuell" kopieren -> Textarea fokussieren, selektieren, execCommand('copy')
      const ta = el.result;
      if (!ta) return;

      // iOS: focus + select ist entscheidend
      ta.focus();
      ta.select();
      // iOS braucht manchmal explizit range
      ta.setSelectionRange(0, ta.value.length);

      const ok = document.execCommand("copy");
      // Auswahl wieder aufheben (optional)
      ...
      alert("Kopieren nicht möglich: " + (err?.message || String(err)));
    }
  }
  ...
  function init() {
    ...
    el.btnCopy?.addEventListener("click", (ev) => copyResult(ev));
