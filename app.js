...
(() => {
  ...
  function copyResult() {
    try {
      ...
      // Copy exactly like manual selection-copy (most reliable on iOS Notes/Mail/Outlook)
      ta.focus();
      ta.select();
...
/* --- COPY BUTTON OVERRIDE (FULL, HARD STOP) --- */
/* Forces iOS to copy exactly like manual selection and prevents any other copy handlers */
(function () {
  ...
  document.addEventListener(
    "click",
    function (e) {
      const btn = e.target && (e.target.closest ? e.target.closest("#btnCopy") : null);
      if (!btn) return;

      // Stop all other handlers (capture phase)
      e.preventDefault();
