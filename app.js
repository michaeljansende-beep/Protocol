...
(() => {
  ...
  async function copyResult() {
    try {
      ...
      // 1) If something ended up URL-encoded, decode it once
      t = tryUrlDecodeOnce(t);

      // 2) Always prevent iOS/Apps from turning content into links
      t = breakDataDetectors(t);

      // 3) Prefer plain text clipboard write, but fall back to execCommand for iOS stability
      try {
