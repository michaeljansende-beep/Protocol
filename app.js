...
(() => {
  ...
  // Bricht typische Link-Erkennung (Notizen/WhatsApp/Mail/Word) ohne sichtbare Änderungen.
  // Hinweis: eingefügte Zero-Width-Spaces können Links nicht mehr klickbar machen.
  function deLinkify(text) {
    const ZWSP = "\u200B";
    let t = String(text || "");

    // 1) http(s):// -> h t t p s :// (ZWSP nach ":")
    t = t.replace(/https?:\/\//gi, (m) => m.replace("://", `:${ZWSP}//`));

    // 2) www. -> ww<wsp>w.
    t = t.replace(/\bwww\./gi, `ww${ZWSP}w.`);

    // 3) E-Mail -> @<wsp>
    t = t.replace(/@/g, `@${ZWSP}`);

    // 4) nackte Domains wie example.de / foo-bar.com (ohne http/www) entschärfen
    //    Wir fügen ZWSP nach dem ersten Punkt ein: example.<wsp>de
    t = t.replace(
      /\b([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)\.((?:de|com|net|org|eu|info|io|co|biz|me|dev|app|shop|online|store|cloud|systems|solutions|gmbh|at|ch|fr|it|es|nl|be|dk|se|no|fi|pl|cz|sk|hu|ro|bg|hr|si|rs|ba|me|mk|al|gr|pt|ie|uk))\b/gi,
      (_m, a, b) => `${a}.${ZWSP}${b}`
    );
  ...
  async function copyResult() {
    ...
    // Für Notizen/WhatsApp/Mail/Word: Link-Erkennung brechen (sonst wird Einfügen oft blau/als Link angezeigt)
    const toCopy = deLinkify(t);

    // 1) Modern: ClipboardItem nur text/plain (kein HTML)
    try {
    ...
    // 2) Fallback: execCommand(copy) - auf iOS oft stabil im Button-Tap Kontext
    try {
