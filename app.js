...
// iOS/Safari can fire multiple input events for a single tap. Use a short lock,
// but also auto-unlock after a safety timeout so the button never becomes "dead".
let recordTapLock = false;
let recordTapLockedAt = 0;
function lockRecordTap(ms = 350) {
  const now = Date.now();
  // Safety: if for any reason the unlock timer didn't run, unlock after 1.2s.
  if (recordTapLock && now - recordTapLockedAt > 1200) {
    recordTapLock = false;
  }
  if (recordTapLock) return true;
  recordTapLock = true;
  recordTapLockedAt = now;
  window.setTimeout(() => {
    recordTapLock = false;
  }, ms);
  return false;
...
// Unified toggle for the record button.
// Keep this as a small wrapper so missing functions never break the UI.
async function toggleRecording(el) {
  try {
    if (isRecording) {
      stopRecording(el);
    } else {
...
function init() {
  ...
  if (el.btnRecord) {
    const onRecordPress = (ev) => {
      // Keep default browser behavior, just avoid accidental double triggers.
      if (lockRecordTap()) return;
      toggleRecording(el);
    };

    // Use pointer events when available (best on iOS/iPadOS), otherwise click.
    if (window.PointerEvent) {
      el.btnRecord.addEventListener("pointerup", onRecordPress);
    } else {
