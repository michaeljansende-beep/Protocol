...
// --- Helpers for copy/paste behavior (iOS Notes/WhatsApp/Mail/Word) ---

function maybeDecodeURIComponent(s) {
  const str = String(s || "");
  // If it contains typical percent-encoding patterns, try to decode.
  if (!/%[0-9A-Fa-f]{2}/.test(str)) return str;
  try {
    return decodeURIComponent(str);
  } catch {
    return str; // keep original if decoding fails
  }
}

function deLinkify(text) {
  // Break auto-linking heuristics by inserting zero-width spaces into typical link patterns.
  // Result stays visually identical, but is no longer detected as a URL/email by most apps.
  const ZWSP = "\u200B";
  return String(text || "")
    .replace(/https?:\/\//gi, (m) => m.replace("://", `:${ZWSP}//`))
    .replace(/\bwww\./gi, `ww${ZWSP}w.`)
    .replace(/@/g, `@${ZWSP}`)
    // Dots are a strong signal for domains/emails; break them as well.
    .replace(/\./g, `.${ZWSP}`);
}
  ...
  // 1) If the text looks URL-encoded (e.g. contains %0A, %20, %C3%A4), decode it first.
  const decoded = maybeDecodeURIComponent(raw);

  // 2) Always prevent auto-linking in Notes/WhatsApp/Mail/Word.
  const toCopy = deLinkify(decoded);

  // 3) Copy as plain text only (iOS/Safari friendly) with a robust fallback.
  try {
    if (navigator.clipboard?.write && window.ClipboardItem) {
      ...
      // Plain-text API (still fine), keep as fallback.
    ...
    // Fallback: execCommand works in many iOS contexts (button tap).
    try {
