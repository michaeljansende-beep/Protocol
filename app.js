...
(() => {
  ...
  // --- Anti-Linkify (immer ohne Links in Notizen/WhatsApp/Mail/Word) ---
  ...
  // Harte Variante: optisch nahezu identisch, aber Apps erkennen es nicht mehr als Link.
  // (Bewusst: Links/E-Mails werden dadurch nicht klickbar.)
  function hardNoLinks(s) {
    return String(s)
      .replace(/%/g, "％")
      .replace(/:/g, "：")
      .replace(/\//g, "／")
  ...
  async async function copyResult() {
    try {
      let t = (el.result?.value || "").trim();
      if (!t) return;

      // Falls jemand encoded Text im Ergebnis hat: zuerst dekodieren
      t = tryUrlDecodeOnce(t);

      // Immer ohne Links: harte Variante gegen iOS/Apps Auto-Linkify
      t = hardNoLinks(t);

      // iOS/Safari: bevorzugt nur text/plain
      try {
      ...
      // Fallback: execCommand (oft stabiler in iOS Button-Tap Kontext)
