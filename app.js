...
// Recording state machine to avoid iOS ghost-taps causing an immediate restart after STOP.
let recordState = "idle"; // "idle" | "recording" | "stopping"
...
function setRecordingUI(isRecording) {
  ...
  if (btn && btn === el.btnRecToggle) {
    // In manchen index-Versionen gibt es kein separates Label (recLabel).
    // Dann muss der Buttontext selbst umschalten, sonst sieht man keinen "Stop".
    const btnText = isRecording ? "Stop" : "Aufnahme starten";
    if (el.recLabel) el.recLabel.textContent = isRecording ? "Aufnahme läuft" : "Aufnahme starten";
    // Falls der Button bereits ein inneres Label-Element hat, nur das ersetzen.
    const inner = btn.querySelector(".btn-text, [data-role='btnText']");
    if (inner) inner.textContent = btnText;
  ...
  if (btn && btn === el.btnRecord) {
    // Wenn es keinen dedizierten Stop-Button gibt, muss der Start-Button als Toggle funktionieren.
    const hasDedicatedStop = !!stopBtn;

    btn.classList.toggle("recording", isRecording);
    btn.classList.toggle("rec", true);

    // Rot-Anzeige (auch im Nicht-Aufnahme-Zustand soll der Button klar als "Record" erkennbar sein)
    if (isRecording) {
      btn.dataset._bg = btn.style.backgroundColor || "";
      btn.dataset._color = btn.style.color || "";
      btn.style.backgroundColor = "#b10f0f";
      ...
      // Standard: ebenfalls rot, aber "Record"-Text
      btn.style.backgroundColor = "#b10f0f";
      btn.style.color = "#fff";
    ...
    if (hasDedicatedStop) {
      // Klassische UI: Start wird während Aufnahme deaktiviert, Stop aktiviert
      btn.disabled = isRecording;
      stopBtn.disabled = !isRecording;
    } else {
      // Toggle UI: Button muss klickbar bleiben, um die Aufnahme zu stoppen.
      btn.disabled = false;
    }
  }
...
// Temporarily lock/unlock recording controls (prevents iOS ghost-taps from re-triggering start after stop).
...
async function stopRecording() {
  ...
  // Immediately flip state/UI to avoid iOS ghost-tap causing an instant restart.
...
async function toggleRecording() {
  // iOS may fire a ghost 2nd click after a tap.
  // Guarding prevents STOP -> immediate START.
  if (recordTapLock) return;
  recordTapLock = true;
  setTimeout(() => (recordTapLock = false), 450);

  // Respect state machine
  if (recordState === "stopping") return;
  if (isRecordingActive || recordState === "recording") {
    await stopRecording();
  } else {
...
function init() {
  ...
  // Audio wiring (alt)
  if (el.btnRecord) {
    // Falls es keinen separaten Stop-Button gibt, muss der Start-Button togglen.
    if (el.btnStop) {
