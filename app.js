...
(() => {
  ...
  function tryUrlDecodeOnce(s) {
    const t = String(s ?? "");
    if (!/%[0-9A-Fa-f]{2}/.test(t)) return t;
    try { return decodeURIComponent(t); } catch { return t; }
  }

  // Break iOS "Data Detectors" (URLs, emails, phone numbers) so pasted text is not turned into links.
  // Uses zero-width spaces that are invisible but disrupt patterns.
  function breakDataDetectors(s) {
    const Z = "​"; // zero-width space
    return String(s ?? "")
      // common URL starters
      .replace(/https?:\/\//gi, (m) => m.split("").join(Z))
      .replace(/\bwww\./gi, (m) => m.split("").join(Z))
      // email trigger
      .replace(/@/g, `@${Z}`)
      // URL/domain separators
      .replace(/\./g, `.${Z}`)
      .replace(/\//g, `/${Z}`)
      .replace(/:/g, `:${Z}`)
      // phone-number / numeric pattern disruption
      .replace(/\d/g, (d) => `${d}${Z}`)
      .replace(/-/g, `-${Z}`);
  }
  ...
  async function copyResult() {
    try {
      let t = (el.result?.value || "").trim();
      if (!t) return;

      // 1) If something ended up URL-encoded, decode it (once or twice if needed)
      t = tryUrlDecodeOnce(t);
      t = tryUrlDecodeOnce(t);

      // 2) Always prevent iOS Notes/Mail from turning content into links (Data Detectors)
      // - first: invisible breaks
      t = breakDataDetectors(t);

      // - second: hard breaks for URL-like tokens (iOS Mail can ignore ZWSP in some cases)
      t = String(t)
        .replace(/%/g, "％")
        .replace(/:/g, "：")
        .replace(/\//g, "／")
      ...
      // 3) Force *plain text only* into clipboard.
      // Use the textarea/execCommand path to avoid iOS providing a URL-flavored clipboard item
      // that Notes/Mail might prefer over text/plain.
