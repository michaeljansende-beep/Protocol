...
(() => {
  ...
  function maybeDecodeURIComponent(raw) {
    const s = String(raw ?? "");
    if (!/%[0-9A-Fa-f]{2}/.test(s)) return s;
    if (!/(%0A|%0D|%20|%3A|%C3)/i.test(s)) return s;
  ...
  async function createProtocol() {
    try {
      ...
      const rawText = (data.protocol || data.protocolText || data.text || "").trim();
      if (!rawText) throw new Error("Antwort war leer.");

      const text = maybeDecodeURIComponent(rawText);
  ...
  async function copyResult() {
    try {
      ...
      let text = maybeDecodeURIComponent(raw);
      text = deLinkify(text);
