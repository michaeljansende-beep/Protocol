...
(() => {
  ...
  // iOS Notes/Mail: Data Detectors sind extrem aggressiv.
  // Diese Funktion fügt unsichtbare WORD JOINER Zeichen ein, sodass nichts mehr als Link/Telefon/Mail erkannt wird.
  function ultraNoLinks(text) {
    const WJ = "\u2060"; // WORD JOINER (unsichtbar)
    const s = String(text ?? "");
    let out = "";
    for (const ch of s) {
      out += ch;
      // Leerzeichen/Zeilenumbrüche nicht aufblasen, alles andere trennen
      if (ch !== " " && ch !== "\n" && ch !== "\r" && ch !== "\t") out += WJ;
    }
    return out;
  }
  ...
  async function copyResult() {
    try {
      ...
      // Immer ohne Links: iOS Notes/Mail sollen keinerlei Muster erkennen.
      t = ultraNoLinks(t);

      // Plaintext-Kopie (stabil auf iOS Safari)
