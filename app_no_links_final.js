...
(() => {
  ...
  // --- Clipboard helpers (always plain text, never links) ---
  function maybeDecodeUrlEncoded(s) {
    let t = String(s ?? "");
    // quick heuristic: %HH sequences or common encoded newlines/spaces
    const looksEncoded = /%[0-9A-Fa-f]{2}/.test(t) || /%0A|%0D|%20|%C3/i.test(t);
    if (!looksEncoded) return t;

    // Some encoders use "+" for spaces (rare here, but harmless)
    t = t.replace(/\+/g, "%20");

    // Decode once or twice (avoid throwing)
    for (let i = 0; i < 2; i++) {
      try {
  ...
  function deLinkifyAlways(text) {
    // Break iOS link detection reliably by inserting zero‑width spaces into common URL/email patterns.
    const ZWSP = "\u200B";
    return String(text ?? "")
      // break percent-encoded "URL looking" strings (%0A, %20, %C3%A4, ...)
      .replace(/%/g, `%${ZWSP}`)
      // break schemes / www
      .replace(/\bhttps?:\/\//gi, (m) => m.replace("://", `:${ZWSP}//`))
      .replace(/\bwww\./gi, (m) => `ww${ZWSP}w.`)
      // break email + domains
      .replace(/@/g, `@${ZWSP}`)
      .replace(/\./g, `.${ZWSP}`);
  }
function buildMetaText() {
  ...
  async function copyResult() {
    ...
    // 1) If text looks URL-encoded, decode it first (fixes %0A/%20/%C3… cases)
    const decoded = maybeDecodeUrlEncoded(raw);

    // 2) Always prevent linkification in Notes/WhatsApp/Mail/Word
    const toCopy = deLinkifyAlways(decoded);

    // 3) Write as plain text only (no HTML). Use iOS-stable fallback.
    try {
    ...
    try {
      ...
      if (ok) alert("In Zwischenablage kopiert.");
      else throw new Error("execCommand(copy) fehlgeschlagen");
    } catch (e) {
