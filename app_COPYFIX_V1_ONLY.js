...
(() => {
  ...
  async function copyResult() {
    // COPYFIX_V1: iOS Notes/Mail friendly copy (same as manual select+copy)
    try {
      const ta = el.result;
      const t = (ta?.value || "").trim();
      if (!t) return;

      // Prefer manual-selection copy path (works better in Apple Notes/Mail/Outlook)
      const wasReadOnly = ta && ta.hasAttribute && ta.hasAttribute("readonly");
